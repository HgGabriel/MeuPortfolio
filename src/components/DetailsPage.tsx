    ...
    <main className="pt-16">
      ...
      <section id="personal" className="relative overflow-hidden pt-[70px] pr-[90px] pb-[60px] pl-[120px]">
        <div className="mx-auto w-full max-w-[1180px]">
          ...
          <p className="mb-14 max-w-[560px] text-[17px] leading-[1.8] text-muted">
            Código é uma grande paixão, mas não a única. Três coisas dão ritmo aos meus dias fora
            do teclado:
          </p>
          ...
          <div className="grid grid-cols-3 gap-7">
            {hobbies.map((h) => (
              ...
              >
                ...
                <div className="mb-2 font-oswald text-xs tracking-[2px] text-accent">{h.tag}</div>
                <h3 className="mb-3 font-poppins text-[22px] font-semibold text-fg">{h.title}</h3>
                <p className="m-0 text-[15px] leading-[1.75] text-muted">{h.body}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-10 flex items-center gap-7 rounded-[20px] border border-line bg-linear-[120deg,rgba(229,50,42,0.14),rgba(229,50,42,0.03)] px-10 py-[34px]">
            ...
            <div>
              ...
              <div className="mt-1 text-[15px] text-muted">
                Treinando um quilômetro de cada vez para os 15 km mais famosos do Brasil, em 31 de
                dezembro.
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-[30px]">
            {projects.map((p) => (
              ...
              >
                ...
                <div>
                  <h3 className="mb-3 font-poppins text-[26px] font-semibold text-fg">{p.name}</h3>
                  <p className="mb-4 text-[15.5px] leading-[1.75] text-muted">{p.long}</p>
